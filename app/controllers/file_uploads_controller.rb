class FileUploadsController < ApplicationController
  ...
  def index
    ...
    @file = LazyHighCharts::HighChart.new('graph') do |f|
      f.title(:text => "Product Categories")
      f.xAxis(:categories => Product.group('client').count.keys)
      f.series(:name => "Number", :yAxis => 0, :data => Product.group('client').count.values)
